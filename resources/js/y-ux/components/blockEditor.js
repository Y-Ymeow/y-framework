export const BlockEditor = {
    ...
    init() {
        document.querySelectorAll('.ux-block-editor[data-editor-id]').forEach(el => {
            if (!this.editors.has(el.dataset.editorId)) {
                this.initEditor(el);
            }
        });
    },
    ...
    initEditor(wrapperEl) {
        ...
        if (initialBlocks) {
            try {
                state.blocks = JSON.parse(initialBlocks);
            } catch (e) {
                state.blocks = [];
            }
        }

        if (state.blocks.length === 0 && placeholder) {
            state.blocks.push({
                blockName: 'paragraph',
                attributes: { content: '' }
            });
    ...
    renderBlock(editorId, block, index) {
        ...
        toolbar.innerHTML = `
            <span class="ux-block-editor__block-type">${blockDef ? blockDef.title : block.blockName}</span>
            <div class="ux-block-editor__block-actions">
                <button type="button" class="ux-block-editor__block-btn" data-action="move-up" title="上移">↑</button>
                <button type="button" class="ux-block-editor__block-btn" data-action="move-down" title="下移">↓</button>
                <button type="button" class="ux-block-editor__block-btn" data-action="delete" title="删除">✕</button>
            </div>
        `;
        ...
        wrapper.addEventListener('click', () => {
            this.selectBlock(editorId, index);
        });
    ...
    renderBlockContent(container, block, blockDef) {
        if (!blockDef) {
            container.innerHTML = '<div class="ux-block-editor__block-unknown">未知 Block: ' + block.blockName + '</div>';
            return;
        }
        ...
        switch (block.blockName) {
            case 'paragraph':
                this.renderParagraphBlock(container, attrs);
                break;
            case 'heading':
                this.renderHeadingBlock(container, attrs);
                break;
            case 'image':
                this.renderImageBlock(container, attrs);
                break;
            case 'quote':
                this.renderQuoteBlock(container, attrs);
                break;
            case 'code':
                this.renderCodeBlock(container, attrs);
                break;
            case 'list':
                this.renderListBlock(container, attrs);
                break;
            case 'divider':
                this.renderDividerBlock(container);
                break;
            default:
                this.renderGenericBlock(container, block, blockDef);
                break;
        }
    },
    ...
    renderImageBlock(container, attrs) {
        ...
        if (attrs.src) {
            ...
            img.src = attrs.src;
            img.alt = attrs.alt || '';
            img.className = 'ux-block-editor__image-preview';
            wrapper.appendChild(img);
        }

        const srcInput = document.createElement('input');
        srcInput.type = 'text';
        srcInput.className = 'ux-block-editor__input';
        srcInput.placeholder = '图片地址';
        srcInput.value = attrs.src || '';
        srcInput.dataset.attr = 'src';
        this.bindInput(srcInput);
        wrapper.appendChild(srcInput);

        const altInput = document.createElement('input');
        altInput.type = 'text';
        altInput.className = 'ux-block-editor__input';
        altInput.placeholder = '替代文字';
        altInput.value = attrs.alt || '';
        altInput.dataset.attr = 'alt';
        this.bindInput(altInput);
        wrapper.appendChild(altInput);

        const captionInput = document.createElement('input');
        captionInput.type = 'text';
        captionInput.className = 'ux-block-editor__input';
        captionInput.placeholder = '图片说明';
        captionInput.value = attrs.caption || '';
        captionInput.dataset.attr = 'caption';
        this.bindInput(captionInput);
    ...
    renderQuoteBlock(container, attrs) {
        ...
        citeInput.type = 'text';
        citeInput.className = 'ux-block-editor__input';
        citeInput.placeholder = '引用来源';
        citeInput.value = attrs.citation || '';
        citeInput.dataset.attr = 'citation';
        this.bindInput(citeInput);
    ...
    renderCodeBlock(container, attrs) {
        ...
        langInput.type = 'text';
        langInput.className = 'ux-block-editor__input';
        langInput.placeholder = '语言（如 php, js）';
        langInput.value = attrs.language || '';
        langInput.dataset.attr = 'language';
        this.bindInput(langInput);
    ...
    renderListBlock(container, attrs) {
        ...
        orderedCheck.checked = attrs.ordered || false;
        ...
        items.forEach((item, i) => {
            listEl.appendChild(this.createListItem(item, i));
        });

        const addBtn = document.createElement('button');
        addBtn.type = 'button';
        addBtn.className = 'ux-block-editor__list-add';
        addBtn.textContent = '+ 添加列表项';
        addBtn.addEventListener('click', () => {
            const index = listEl.querySelectorAll('.ux-block-editor__list-item').length;
            listEl.appendChild(this.createListItem('', index));
    ...
    createListItem(text, index) {
        ...
        input.type = 'text';
        input.className = 'ux-block-editor__input';
        input.value = text;
        input.dataset.itemIndex = index;
        input.addEventListener('input', () => this.syncListItems(itemEl.closest('.ux-block-editor__list-items')));

        const removeBtn = document.createElement('button');
        removeBtn.type = 'button';
        removeBtn.className = 'ux-block-editor__list-item-remove';
        removeBtn.textContent = '✕';
        removeBtn.addEventListener('click', () => {
            itemEl.remove();
            this.syncListItems(itemEl.closest('.ux-block-editor__list-items'));
        });
    ...
    syncListItems(listEl) {
        ...
        const items = [];
        listEl.querySelectorAll('.ux-block-editor__list-item input').forEach(input => {
    ...
    renderGenericBlock(container, block, blockDef) {
        ...
        for (const [attrName, attrDef] of Object.entries(attrDefs)) {
            ...
            if (type === 'boolean') {
                ...
                input.className = 'ux-block-editor__editable';
                input.contentEditable = true;
                input.dataset.attr = attrName;
                input.innerHTML = value || '';
                this.bindEditable(input);
            } else if (type === 'number') {
                input = document.createElement('input');
                input.type = 'number';
                input.className = 'ux-block-editor__input';
    ...
    bindEditable(el) {
        ...
        el.addEventListener('blur', () => {
            if (!el.textContent.trim()) {
                ...
                el.textContent = this.getPlaceholder(blockName);
            }
    ...
    bindSelect(select) {
        select.addEventListener('change', () => {
            this.syncFromSelect(select);
        });
    },

    bindCheckbox(checkbox) {
        checkbox.addEventListener('change', () => {
            this.syncFromCheckbox(checkbox);
        });
    },
    ...
    getPlaceholder(blockName) {
        ...
        return placeholders[blockName] || '输入内容...';
    },
    ...
    syncFromSelect(select) {
        ...
        if (editor && editor.state.blocks[index]) {
            ...
            editor.state.blocks[index].attributes[attrName] = val;
            this.syncToHidden(editorId);
        }
    },
    ...
    insertBlock(editorId, blockName) {
        ...
        const insertIndex = editor.state.selectedBlockIndex >= 0
            ? editor.state.selectedBlockIndex + 1
    ...
    deleteBlock(editorId, index) {
        ...
        if (editor.state.selectedBlockIndex >= editor.state.blocks.length) {
            editor.state.selectedBlockIndex = editor.state.blocks.length - 1;
        }
    ...
    bindInserter(editorId) {
        ...
        if (toggleBtn && panel) {
            toggleBtn.addEventListener('click', () => {
                panel.classList.toggle('ux-block-editor__inserter-panel--open');
            });
        }
        ...
        editor.wrapperEl.addEventListener('click', (e) => {
            ...
            if (blockName) {
                this.insertBlock(editorId, blockName);
            }
        });
    },
    ...
    getValue(editorId) {
        const editor = this.editors.get(editorId);
        return editor ? JSON.stringify(editor.state.blocks) : '';
    },
    ...
    setValue(editorId, value) {
        ...
        try {
            editor.state.blocks = JSON.parse(value);
        } catch (e) {
            editor.state.blocks = [];
        }
