...
directive('model', (el, state, method, { content, effect, execute, $execute }) => {
...
directive('on', (el, state, method, { content, modifiers, $execute }) => {
    ...
    if (modifiers.includes('throttle')) {
        let lastTime = 0;
        const delay = parseInt(modifiers[modifiers.indexOf('throttle') + 1]) || 300;
        wrappedHandler = (e) => {
            const now = Date.now();
    ...
    if (modifiers.includes('outside')) {
        ...
        return () => document.removeEventListener(eventName || 'click', outsideHandler);
    }
